...
This document is a guide for adding or updating site locations to the Choptank database. The database uses the ODM2 framework schema and vocabulary. 
...
wetlands <- readOGR(file.path(path, "SamplingFeature_sites.kmz"))
```

# Match column names to database table

```{r}
new_samp_features <- data.frame("samplingfeaturecode" = wetlands$Name,
                                "samplingfeaturename" = wetlands$description,
                                "samplingfeaturetypecv" = "site",
                                "featuregeometrywkt" = writeWKT(wetlands, byid = TRUE),
...
wetlands is a spatial polygons data frame

dataframe contains Name and description from google earth file
...
make data frame to append to sampling features table
