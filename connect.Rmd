...
The `choptank` database is hosted on the sesync postgis database server. You can connect using the `RPostgreSQL` package when you are on the network at SESYNC, logged into VPN, or through the SESYNC RStudio server at [rstudio.sesync.org](https://rstudio.sesync.org/) with your sesync log-in credentials.
...
Then use the `dbConnect()` function to establish a connection "object" to the database, which will allow sending and receiving information from the database. 
...
If you are writing SQL commands, use the format `schema.table` to refer to tables:
...
If you have multiple lines of SQL, make sure to remove line endings before executing the SQL. 
...
To see the whole SQL statement, save sql character string as a text file.
